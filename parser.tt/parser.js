...
var cache = {},
  ...
  fs = tt.getFileSystemManager && tt.getFileSystemManager();
var dom;
var search;
// 计算 cache 的 key
function hash(str) {
...
Component({
  ...
  created() {
    ...
    this.imgList.setItem = function (i, src) {
      ...
      if (src.includes('data:image')) {
        ...
        if (!info) return;
        var filePath = `${tt.env.USER_DATA_PATH}/${Date.now()}.${info[1]}`;
        fs && fs.writeFile({
          filePath,
          data: info[3],
          encoding: info[2],
  ...
  detached() {
    ...
    this.imgList.each(src => {
      if (src && src.includes(tt.env.USER_DATA_PATH) && fs)
        fs.unlink({
          filePath: src
        })
    })
  ...
  methods: {
    ...
    navigateTo(obj) {
      ...
      var selector = tt.createSelectorQuery().in((this._in ? this._in.page : this)).select((this._in ? this._in.selector : '._top') + (obj.id ? '>>>#' + obj.id : '')).boundingClientRect();
        ...
        for (let i = 0, n; n = ns[i++];) {
          ...
          for (var j = 0, item; item = n.data.nodes[j++];) {
            ...
            // 音视频控制
              ...
              var ctx;
              if (item.name == 'video') ctx = tt.createVideoContext(item.attrs.id, n);
