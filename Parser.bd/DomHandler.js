...
function initStyle(tagStyle) {
    ...
    tagStyle.blockquote = tagStyle.blockquote || 'margin:0;background-color:#f6f6f6;border-left:3px solid #dbdbdb;color:#6c6c6c;padding:5px 0 5px 10px;';
...
DomHandler.prototype.onopentag = function (name, attrs) {
    ...
    if (attrs.id) matched += this._style['#' + attrs.id] ? this._style['#' + attrs.id] + ';' : '';
    if (attrs.class) {
    ...
    switch (name) {
        ...
        case 'img':
            ...
            if (!attrs.hasOwnProperty('ignore') && attrs.src) {
                if (this.imgList.indexOf(attrs.src) != -1) attrs.src = attrs.src + "?index=" + this.imgIndex++;
        ...
        case 'audio':
            ...
            } else attrs.style = "max-width:100%;" + attrs.style;
...
DomHandler.prototype.onclosetag = function (name) {
    ...
    let parent = this._tagStack.length ? this._tagStack[this._tagStack.length - 1] : null;
    // 合并一些不必要的层，减小节点深度
    if (element.children.length == 1 && element.name == 'div' && element.children[0].name == 'div' && !/padding/.test(element.attrs.style) && !(/margin/.test(element.attrs.style) && /margin/.test(element.children[0].attrs.style)) && !/display/.test(element.attrs.style) && !/display/.test(element.children[0].attrs.style) && !(element.attrs.id && element.children[0].attrs.id)) {
        ...
        if (/padding/.test(element.children[0].attrs.style)) element.children[0].attrs.style = ";box-sizing:border-box;" + element.children[0].attrs.style;
    ...
    if (element.pre) {
        this._whiteSpace = false;
        for (var ele of this._tagStack) if (ele.pre) this._whiteSpace = true;
        delete element.pre;
    }
};
