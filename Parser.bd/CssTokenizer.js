...
CssTokenizer.prototype.InContent = function (c) {
  if (c == '}') {
    ...
    for (let item of this._list) this.res[item] = (this.res[item] || '') + ";" + this._content;
    this._list = [];
    this._comma = false;
    this._state = "SPACE";
  }
...
CssTokenizer.prototype.parse = function () {
  for (; this._index < this._buffer.length; this._index++) this[this._state](this._buffer[this._index]);
  return this.res;
};
