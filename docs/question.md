...
#### 图片加载完时突变 ####
*问题描述：*  
若使用小图（未填满一行），加载完成后会突然缩小；若使用长图，加载完成后会突然变长；某些情况下可能影响体验（尤其是在视图范围内时）  
...
1. 若设置了宽度，没设置高度，则设置 `mode` 为 `widthFix`  
2. 若设置了高度，没设置宽度，则设置 `mode` 为 `heightFix`（`2.10.3` 以上支持，低基础库可能存在问题）  
3. 若同时设置了宽高度，则设置 `mode` 为 `scaleToFill`  
4. 若既没有设置宽度，也没有设置高度，则设置 `mode` 为 `widthFix`，并给这张图设置一个标记，在 `load` 事件中，将宽度设置为原大小（高基础库通过 `wxs` 设置，低基础库通过 `js` 设置；由于 `max-width:100%` 的限制，设置的宽度不一定是实际宽度，因此高度交给 `image` 自动计算）  

这样基本上实现了 `img` 的效果（这是目前想到的性能开销最小的处理方式，如果有更优方案欢迎提出）  
...
- 简单方案  
  大部分情况下，只需要在 `config.js` 的 `filter` 函数中添加以下内容即可（个别情况自行调整）：
  ```javascript
  filter(node) {
...
#### 表格中的链接无法点击 ####
*问题描述：*  
使用了 `colspan` 或 `rowspan` 的表格，其中的链接和图片将无法点击  

*问题原因：*  
因为使用了 `colspan` 或 `rowspan` 的表格无法通过 `css` 模拟，将直接通过 `rich-text` 显示，因此其中的图片、链接无法点击，也无法使用音视频  
...
#### 无法编辑 / 没有配套的编辑器 ####
本插件仅支持显示富文本，没有编辑功能  
本插件没有专门配套的富文本编辑器，一般来说，能够导出 `html` 的富文本编辑器都是支持的  
