...
即总耗时在缓存不命中时约 `135ms`，缓存命中（或直接传入数组）时约 `105ms`  
相关解释：  
1. 缓存是指开启 `use-cache` 属性后，首次解析后会缓存结果，多次打开不会重复解析，可以节省时间（也可以通过 [preLoad](/instructions#preLoad) 提前解析）  
2. 传入数组虽然可以省去解析时间，但一般来说数组序列化后长度比字符串长，可能增加网络传输时间，需酌情考虑（这篇文章字符串时大小为 `39.3KB`，解析结果（数组序列化后）大小为 `55.8KB`）  
