...
## 性能指标  
本插件解析和渲染效率高，轻量化（\`40.6KB\`，\`min\`版本\`25.6KB\`）；封装成自定义组件，简便易用；容错性强，稳定性高，以下情况都不会出错：  
\`\`\`html
<!--冒号不匹配-->
...
- 2020.4.13
  1. \`F\` 修复了自动移除空\`div\`导致换行失效的问题
  2. \`F\` 修复了使用多个并列\`rpx\`时可能失效的问题
  3. \`F\` 修复了\`audio\`扩展包音乐名太长会导致样式错乱的问题
</br></br>
