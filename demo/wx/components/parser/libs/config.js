...
const trustAttrs = makeMap("align,alt,app-id,author,autoplay,border,cellpadding,cellspacing,class,color,colspan,controls,data-src,dir,face,height,href,id,ignore,loop,media,muted,name,path,poster,rowspan,size,span,src,start,style,type,unit-id,unitId,width,xmlns");
// 信任的标签，将保持标签名不变
...
module.exports = {
  // 高亮处理函数
  highlight(content) {
    var info = content.match(/<code[\s\S]+?language-([a-z\-]+)[\s\S]+?>([\s\S]+)<\/code>/);
    ...
    if (!lan.includes("editor"))
      content = `<span style="position:absolute;top:3px;right:8px;font-size:.6rem;color:#808080;font-weight:bold;user-select:none">${lan}</span><div style='overflow:auto;max-width:100%;padding-bottom:1em'>${content}</div>`;
    return content;
  },
  ...
  LabelHandler(node, Parser) {
    ...
    switch (node.name) {
      ...
      case "ad":
        ...
        // 自定义标签
      case "pre":
        if (!attrs.id) attrs.id = "pre" + (++Parser._preNum);
        bubbling(Parser);
        break;
      case "font":
        if (attrs.color) {
