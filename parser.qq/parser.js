...
Component({
  ...
  created() {
    ...
    this.imgList.setItem = function(i, src) {
      ...
      if (src.includes('data:image')) {
        ...
        if (!info) return;
        var filePath = `${qq.env.USER_DATA_PATH}/${Date.now()}.${info[1]}`;
        fs && fs.writeFile({
          filePath,
          data: info[3],
          encoding: info[2],
  ...
  detached() {
    ...
    this.imgList.each(src => {
      if (src && src.includes(qq.env.USER_DATA_PATH) && fs)
        fs.unlink({
          filePath: src
        })
    })
  ...
  methods: {
    ...
    setContent(html, append, _watch) {
      ...
      for (let i = 0, n; n = ns[i++];) {
        ...
        for (var j = 0, item; item = n.data.nodes[j++];) {
          ...
          // 音视频控制
            ...
            var ctx;
            if (item.name == 'video') ctx = qq.createVideoContext(item.attrs.id, n);
      ...
      (qq.nextTick || setTimeout)(() => this.triggerEvent('load'), 50);
      var height;
      clearInterval(this._timer);
